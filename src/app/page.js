            ...
            <div className="px-5 sm:px-10 md:px-10 lg:px-20 mt-0">
                <div className="p-3 flex gap-4 md:gap-y-4 sm:gap-y-4 lg:gap-y-0 justify-between md:flex-col sm:flex-col lg:flex-row flex-col">
                    ...
                    <div className="basis-1/3 flex justify-end gap-3 items-center">
                        <button onClick={() => setGridView(false)} className={`border p-3 ${(!gridView) ? 'bg-black text-white': 'hover:bg-gray-100'} rounded`}><Icon icon="mdi:format-list-bulleted" /></button>
                        <button onClick={() => setGridView(true)} className={`border p-3 ${(gridView) ? 'bg-black text-white': 'hover:bg-gray-100'} rounded`}><Icon icon="mdi:view-grid" /></button>
                    </div>
                </div>
            </div>
                                ...
                                <TableBody>
                                    {
                                        filteredApps.map((ele) => (
                                            <TableRow key="1">
                                                ...
                                                <TableCell>
                                                    <Link className="underline-offset-4 underline" href={ele.dest_link}>
                                                        {ele.title}
                                                    </Link>
                                                </TableCell>
